        ...
        for (size_t oil_id = 0; oil_id < input.m; ++oil_id)
        {
            ...
            for (size_t di = 0; di < input.n - oil.max_i; ++di)
            {
                for (size_t dj = 0; dj < input.n - oil.max_j; ++dj)
                {
                    ...
                    oil_state.top_left_query_volumes[top_left].emplace_back(c);
                }
            }
        }
...
public:
    ...
    bool ans(const vector<size_t> &T)
    {
        if (rem == 0)
        {
            cerr << "!log giveup because rem==0" << endl;
            exit(0);
        }
        --rem;
        cout << "a " << T.size() << " ";
    ...
    size_t query(const vector<size_t> &query_coordinates)
    {
        ...
        for (size_t i = 0; i < ln_pr_if_s.size() - 1; ++i)
        {
            if (!isinf(ln_pr_if_s[i]) && isinf(ln_pr_if_s[i + 1]))
            {
                ln_pr_if_s[i + 1] = ln_pr_if_s[i] - 10.0;
            }
        }
        for (size_t i = ln_pr_if_s.size() - 1; i > 0; --i)
        {
            if (!isinf(ln_pr_if_s[i]) && isinf(ln_pr_if_s[i - 1]))
            {
                ln_pr_if_s[i - 1] = ln_pr_if_s[i] - 10.0;
        ...
        ln_pr_if_s_query.emplace_back(ln_pr_if_s);
        return ret;
    }
    ...
    void giveup()
    {
        cerr << "!log giveup because timeup" << endl;
        cout << "!log giveup because timeup" << endl;
...
struct Query
{
    ...
    Query(const Input &input) : in_query(input.n2, false), volume(pool.size(), 0), coordinate_size(0)
    {
    }
    ...
    const Input &input, Sim &sim)
{
    ...
    for (size_t ij = 0; ij < input.n2; ++ij)
    {
        if (same[ij])
        {
            no_info_coordinates.emplace_back(ij);
        }
    }
...
inline void sort_pool(vector<OilLayout> &pool)
{
...
void simulated_annealing(
    ...
    const Sim &sim,
    State &state,
    const vector<vector<vector<pair<size_t, size_t>>>> &swaps,
    ...
    size_t ITER)
{
    ...
    ITER = static_cast<size_t>(ITER * min(time_limit - get_time(), 1.0));
    for (size_t t = 0; t < ITER; ++t)
    {
        double temp = T0 + (T1 - T0) * t / (double)ITER;
        size_t slide_threshold = 30;
        size_t warp_threshold = slide_threshold + 10;
        size_t swap_threshold = warp_threshold + 60;
        size_t coin = rng.randrange(swap_threshold);
        if (coin < slide_threshold)
        {
            // ポリオミノをランダムに選び、上下左右に1マス移動
            size_t oil_id = rng.randrange(input.m);
            auto &oil = input.oils[oil_id];
            auto [di, dj] = DIJ[rng.randrange(4)];
            size_t i2 = state.top_lefts[oil_id] / input.n + di;
            size_t j2 = state.top_lefts[oil_id] % input.n + dj;
            if (i2 < input.n - oil.max_i && j2 < input.n - oil.max_j)
            {
                ...
                if (!hash_ln_lilelihood.count(state.hash))
                {
                    ...
                    {
                        pool.emplace_back(state.hash, next, 0.0, state.top_lefts, state.volumes);
                    }
                }
        ...
        else
        {
            ...
            size_t oil_id_a = rng.randrange(input.m);
            auto &oil_a = input.oils[oil_id_a];
            size_t oil_id_b = rng.randrange(input.m);
            auto &oil_b = input.oils[oil_id_b];
            if (oil_id_a == oil_id_b)
            {
                continue;
            }
            size_t ai = state.top_lefts[oil_id_a] / input.n;
            size_t aj = state.top_lefts[oil_id_a] % input.n;
            size_t bi = state.top_lefts[oil_id_b] / input.n;
            size_t bj = state.top_lefts[oil_id_b] % input.n;
            auto [dai, daj] = swaps[oil_id_b][oil_id_a][rng.randrange(swaps[oil_id_a][oil_id_b].size())];
            auto [dbi, dbj] = swaps[oil_id_a][oil_id_b][rng.randrange(swaps[oil_id_b][oil_id_a].size())];
            size_t ai2 = bi + dai;
            size_t aj2 = bj + daj;
...
int main()
{
    ...
    auto swaps = get_swaps(input);

    Sim sim(input);
    State state(input);
    size_t ITER = 4'000'000 * 5 / (2 * input.n2);
    if (input.is_difficult())
    {
        ITER /= 5;
    }
    ...
    for (size_t t = 0;; ++t)
    {
        ...
        if (best_pool_prob > 0.8)
        {
            ...
            vector<size_t> T_vec;
            vector<size_t> T_vec_reverse;
            for (size_t ij = 0; ij < input.n2; ++ij)
            {
                if (best_bits[ij])
                {
                    T_vec.emplace_back(ij);
                }
                else
                {
                    T_vec_reverse.emplace_back(ij);
                }
            }
            if (
                !input.is_difficult() ||
                (sim.queries.size() > 0 && sim.queries.back().first == T_vec_reverse))
            {
                if (sim.ans(T_vec))
                {
                    break;
        ...
        {
            auto query_coordinates = getDivinationQuery(input, sim);
            // 占いの評価値の方が高い場合、占いを行う
